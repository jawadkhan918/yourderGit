...
- (void)baseInit {
    _notSelectedStar = [UIImage imageNamed:@"star_unfilled"];
    _selectedStar = [UIImage imageNamed:@"star_filled"];
    _halfSelectedStar = [UIImage imageNamed:@"unfilled"];
...
- (void)layoutSubviews
{
    ...
    //float desiredImageWidth = (self.frame.size.width+10 - (_leftMargin*2) - (_midMargin*_starViews.count)) / _starViews.count;
    
    /*
    float imageWidth = 25.0; //14.0; //MAX(_minStarSize.width, desiredImageWidth);
    float imageHeight = 25.0; //14.0;// MAX(_minStarSize.height, self.frame.size.height);
    float ipimageWidth = 30.0; //MAX(_minStarSize.width, desiredImageWidth);
    float ipimageHeight = 30.0;// MAX(_minStarSize.height, self.frame.size.height);
    */
    
    float imageWidth = self.starSize.width; //14.0; //MAX(_minStarSize.width, desiredImageWidth);
    float imageHeight = self.starSize.height; //14.0;// MAX(_minStarSize.height, self.frame.size.height);
    float ipimageWidth = self.starSize.width + 5; //MAX(_minStarSize.width, desiredImageWidth);
    float ipimageHeight = self.starSize.height + 5;// MAX(_minStarSize.height, self.frame.size.height);
    
    if (self.israte== YES) {
        for (int i = 0; i < _starViews.count; ++i)
        {
            imageView = [_starViews objectAtIndex:i];
            CGRect imageFrame = CGRectMake(_leftMargin + i*(_midMargin+ipimageWidth), 5, ipimageWidth, ipimageHeight);
    ...
    {
        imageView = [_starViews objectAtIndex:i];
        CGRect imageFrame = CGRectMake(_leftMargin + i*(_midMargin+imageWidth), 5, imageWidth, imageHeight);
        imageView.frame = imageFrame;
    }
    }
}
...
- (void)handleTouchAtLocation:(CGPoint)touchLocation {
    ...
    [self refreshStars];
    NSLog(@"Total Stars: %lu",(unsigned long)_starViews.count);
}
